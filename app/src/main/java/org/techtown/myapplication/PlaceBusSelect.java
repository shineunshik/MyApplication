...
public class PlaceBusSelect extends AppCompatActivity {
    ...
    public void api() throws IOException{
        ...
        try{
            ...
            urlBuilder.append("&" + URLEncoder.encode("routeNo", "UTF-8") + "=" + URLEncoder.encode("GGB208000008", "UTF-8")); /*노선번호 */
            ...
            for (int temp = 0; temp < nList.getLength(); temp++) { //api가 작동을 하지않아 item 갯수가 없어서 for문이 작동을 안함
                ...
                if (nNode.getNodeType() == Node.ELEMENT_NODE) { //노드 type이 같을 경우에 실행
                    ...
                    ob = new Ob_Bus_Select();
                    ob.setEndnodenm(getTagValue("endnodenm", eElement));
                    ob.setEndvehicletime(getTagValue("endvehicletime",eElement));
                    ob.setRouteid(getTagValue("routeid",eElement));
                    ob.setRouteno(getTagValue("routeno",eElement));
                    ob.setRoutetp(getTagValue("routetp",eElement));
                    ob.setStartnodenm(getTagValue("startnodenm",eElement));
                    ob.setStartvehicletime(getTagValue("startvehicletime",eElement));
